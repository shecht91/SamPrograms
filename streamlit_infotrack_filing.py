...
def get_token(username, password, clientid, clientsecret):
    ...
    with requests.Session() as session:
        response = session.post('https://auth.infotrack.com/connect/token', auth=auth,
...
def login_infotrack(username, password, clientid, clientsecret):
    headers = get_token(username, password, clientid, clientsecret)
...
def mapping(lawyerdetail, courtdetails, lawyers, clientref, retailref, fileids, headers, session):
    data = {
    ...
    response = session.post('https://search.infotrack.com/secure/api/v1/mapping', headers=headers, data=data)
    data2 = response.json()
    mapping_url = data2["Url"]
    fileids.clear()
    return mapping_url

def login_to_efile_CA(mapping_url, session):  # this step is required to scrape the case info. You do not need this function if you are just trying to e-file.
    response = session.get(mapping_url)
    html_content = response.content
    if '<title>InfoTrack | E-Filing Login</title>' in html_content:
        return 'fail'

def retrieve_case_tracking_id(clientref, session, headers):
    ...
    response = session.post('https://search.infotrack.com/secure/api/courtfilingla/court/cases', headers=headers,
                            data=data)
...
def open_case(caseid, casenum, session, headers):
    data = {'caseTrackingId': caseid, 'courtName': 'lasc', 'caseNumber': casenum, 'apiType': '1'}
    response = session.get('https://integrated.infotrack.com/CA/CourtFiling/ExistingCase/New', headers=headers,
                            data=data)
    testopen_case = response.text
...
def scrape_case_info(mapping_url, clientref, session, headers):
    result = login_to_efile_CA(mapping_url, session)
    if result == 'fail':
        return 'fail', 'fail', 'fail'
    case_id, case_num = retrieve_case_tracking_id(clientref, session, headers)
    if case_id == 'no case':
        return 'no case', 'no case', 'no case'
    case_info = open_case(case_id, case_num, session, headers)
    return case_id, case_num, case_info

# Function to search for a case number
def search_case_number(fileids, headers):
    ...
    with requests.Session() as session:
        mapping_url = mapping(lawyerdetail, courtdetails, lawyers, clientref, retailref, fileids, headers, session)
        st.session_state.mapping_url = mapping_url
        casenum, caseid, case_info = scrape_case_info(mapping_url, clientref, session, headers)
        if casenum == 'fail':
...
if st.button("Search Case Info"):
    search_case_number(st.session_state.fileids, st.session_state.headers)
